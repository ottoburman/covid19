...
def covid_show(Arg1, Arg2, days, model_d,forecast_len):
    ...
    import numpy as np #Start: Otto Burman's COVID-19 report by Python 27.5.2020
    import pandas as pd
    ...
    popData1 = df_my.iloc[-1]['popData2019'] #last element - does not work 24.6.:name has been changed?
    ...
    plt.plot(x, my_pol_model(x),label='Model(d={}) Explained {}%'.format(dgr,r2))
    ...
    plt.text(nr_lastdays*0.10,sum_deaths1*0.40, 'Population {} \n Death rate {} % '.format(int(popData1), deaths_rate1 ))
    ...
    plt.text(nr_lastdays*0.10, sum_deaths2*0.40, 'Population {} \n Death rate {} % '.format(int(popData2), deaths_rate2))
